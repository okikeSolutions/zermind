...
const callGtag = (...args: Parameters<Window["gtag"]>) => {
...
const CookieBanner: React.FC = () => {
  ...
  function updateConsentState(granted: boolean) {
    try {
      ...
      callGtag("consent", "update", consentState);

      // Set ads_data_redaction based on ad_storage consent
      callGtag("set", "ads_data_redaction", !granted);
  ...
  useEffect(() => {
    try {
      ...
      const consentListeners: ((consent: { [key: string]: boolean }) => void)[] = [];

...
function SelectGroup({ className, ...props }: SelectPrimitive.Group.Props) {
  return (
    <SelectPrimitive.Group
      ...
      {...sx(cn("scroll-my-1 p-1", className))}
      {...props}
    />
  );
}
...
function SelectValue({ className, ...props }: SelectPrimitive.Value.Props) {
  return (
    <SelectPrimitive.Value
      ...
      {...sx(cn("flex flex-1 text-left", className))}
      {...props}
    />
  );
}
  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={<ChevronDownIcon {...sx("size-4 text-muted-foreground opacity-50")} />}
      />
    </SelectPrimitive.Trigger>
  );
}
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      {...sx(cn("text-muted-foreground px-2 py-1.5 text-xs", className))}
      {...props}
    />
  );
}
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      {...props}
    >
      <SelectPrimitive.ItemText {...sx("flex flex-1 shrink-0 gap-2 whitespace-nowrap")}>
        {children}
      </SelectPrimitive.ItemText>
      <SelectPrimitive.ItemIndicator
        render={
          <span
            {...sx("pointer-events-none absolute right-2 flex size-4 items-center justify-center")}
          />
        }
      >
        <CheckIcon {...sx("size-4")} />
...
function SelectSeparator({ className, ...props }: SelectPrimitive.Separator.Props) {
  return (
    <SelectPrimitive.Separator
      ...
      {...sx(cn("bg-border pointer-events-none -mx-1 my-1 h-px", className))}
      {...props}
    />
  );
}
